import React from 'react';
import { View, Text, StyleSheet, TouchableOpacity, SafeAreaView } from 'react-native';
import LinearGradient from 'react-native-linear-gradient';
import { useProfileStore } from '../store/profileStore';

const GameViewer = ({ route, navigation }) => {
  const { gameId } = route.params || {};
  const { theme } = useProfileStore();
  const isDark = theme === 'dark';

  return (
    <SafeAreaView style={[styles.flex1, isDark && styles.darkContainer]}>
      <LinearGradient
        colors={isDark ? ['#0f172a', '#1e1b4b', '#0d071c'] : ['#cffafe', '#dbeafe', '#e0e7ff']}
        style={styles.centerContainer}
      >
        <Text style={styles.emoji}>🚧</Text>
        <Text style={[styles.title, isDark && styles.darkTitle]}>Game Coming Soon!</Text>
        <Text style={[styles.subtitle, isDark && styles.darkSubtitle]}>Game ID: {gameId}</Text>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
          <Text style={styles.backText}>Go Back</Text>
        </TouchableOpacity>
      </LinearGradient>
    </SafeAreaView>
  );
};

const styles = StyleSheet.create({
  flex1: { flex: 1 },
  darkContainer: { backgroundColor: '#0f172a' },
  centerContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 },
  emoji: { fontSize: 60, marginBottom: 16 },
  title: { fontSize: 24, fontWeight: 'bold', color: '#1d4ed8', marginBottom: 8 },
  darkTitle: { color: '#60a5fa' },
  subtitle: { fontSize: 16, color: '#2563eb', marginBottom: 20 },
  darkSubtitle: { color: '#93c5fd' },
  backButton: { backgroundColor: '#2563eb', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 24 },
  backText: { color: 'white', fontWeight: 'bold', fontSize: 16 },
});

export default GameViewer;
